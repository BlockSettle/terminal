   ...
   , const std::shared_ptr<ApplicationSettings> &appSettings)
{
   ...
   case SignContainer::OpMode::Local:
      return std::make_shared<LocalSigner>(logger, appSettings->GetHomeDir()
         , appSettings->get<NetworkType>(ApplicationSettings::netType), port, pwHash
         , appSettings->get<double>(ApplicationSettings::autoSignSpendLimit));

   case SignContainer::OpMode::Remote:
      return std::make_shared<RemoteSigner>(logger, appSettings->GetHomeDir()
         , appSettings->get<QString>(ApplicationSettings::signerHost), port, pwHash);
