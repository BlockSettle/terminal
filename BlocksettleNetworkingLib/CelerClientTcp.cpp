...
CelerClientTcp::CelerClientTcp(const std::shared_ptr<ConnectionManager> &connectionManager, bool userIdRequired)
   : CelerClient(connectionManager->GetLogger(), userIdRequired)
   , connectionManager_(connectionManager)
{
   connect(this, &CelerClient::sendData, this, &CelerClientTcp::onSendData);
}

CelerClientTcp::~CelerClientTcp() = default;

bool CelerClientTcp::LoginToServer(const std::string &hostname, const std::string &port
   , const std::string &login, const std::string &password)
{
   ...
   bool result = CelerClient::LoginToServer(login, password);
   return result;
}

void CelerClientTcp::CloseConnection()
{
   if (connection_) {
   ...
   CelerClient::CloseConnection();
}

void CelerClientTcp::onSendData(CelerAPI::CelerMessageType messageType, const std::string &data)
{
