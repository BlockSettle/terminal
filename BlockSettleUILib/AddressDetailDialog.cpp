   ...
   , wallet_(wallet)
{
   ...
   ui_->labelWalletName->setText(QString::fromStdString(wallet_->name()));
   ...
   if (armory_->state() != ArmoryState::Ready) {
      ...
      const auto &cbLedgerDelegate = [thisPtr](const std::shared_ptr<AsyncClient::LedgerDelegate> &delegate) {
