...
hd::Node::Node(const bs::core::wallet::Seed &seed)
{
...
void hd::Node::initFrom(const bs::core::wallet::Seed &seed)
{
...
bs::core::wallet::Seed hd::Node::seed() const
{
   bs::core::wallet::Seed seed(netType_);
   if (hasPrivKey_) {
...
std::shared_ptr<hd::Node> hd::Nodes::decrypt(const SecureBinaryData &password) const
{
   ...
   for (const auto &node : nodes_) {
      ...
      if (decryptedNode != nullptr) {
         core::wallet::Seed seed(decryptedNode->getNetworkType(), decryptedNode->privateKey());
