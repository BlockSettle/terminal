        ...
        stage('Build apps') {
            parallel {
                ...
                stage('Build MacOSX app') {
                    steps {
                        sh 'ssh admin@${MACOS_HOST_IP} "rm -rf ~/Workspace/terminal"'
                        sh 'ssh admin@${MACOS_HOST_IP} "cd ~/Workspace ; git clone --single-branch --branch ${TAG} git@github.com:BlockSettle/terminal.git ; cd terminal ; git submodule init ; git submodule update ; cd common ; git submodule init ; git submodule update"'
                        sh 'ssh admin@${MACOS_HOST_IP} "export PATH=/usr/local/opt/ccache/libexec:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin ; ccache -s ; cd /Users/admin/Workspace/terminal/Deploy/MacOSX ; security unlock-keychain -p ${MAC_CHAIN_PAS} login.keychain ; ./package.sh -production"'
                        sh "scp admin@${MACOS_HOST_IP}:~/Workspace/terminal/Deploy/MacOSX/BlockSettle.dmg ${WORKSPACE}/terminal/Deploy/BlockSettle.dmg"
                    }
                }
