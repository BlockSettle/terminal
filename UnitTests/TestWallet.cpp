...
TEST_F(TestWalletWithArmory, SignSettlement)
{
   ...
   auto fut1 = promPtr1->get_future();
   const auto &cbUtxo = [promPtr1](ReturnMessage<std::vector<UTXO>> utxos)
   {
      auto&& utxoVec = utxos.get();
      ASSERT_NE(utxoVec.size(), 0);
      promPtr1->set_value(utxoVec[0]);
   };

   envPtr_->armoryConnection()->bdv()->getUTXOsForAddress(
      msAddress.prefixed(), false, cbUtxo);
    auto utxo1 = fut1.get();
